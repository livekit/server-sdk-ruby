...
# tests drive the middleware directly because failover relies on internal
# test-only knobs (force/backoff_base) the service clients don't expose.
RSpec.describe LiveKit::RegionFailoverMiddleware do
  ...
  def call(base, mock = {}, failover: true, force: true)
    ...
    conn.post('livekit.RoomService/CreateRoom') do |req|
      ...
      req.headers['X-Lk-Mock'] = JSON.generate({ skipAuth: true }.merge(mock))
      req.body = ''
    end
  end
